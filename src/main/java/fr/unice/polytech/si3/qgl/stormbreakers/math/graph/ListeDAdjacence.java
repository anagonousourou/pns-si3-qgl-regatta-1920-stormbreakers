...
public class ListeDAdjacence {
	...
	private int VISIBLITEMAX = 1000;//TODO a passer en parametre ou avoir variable global qui nous dit si visibleMax change (ex: vigie activé augmente la visibilité)
	private List<Arrete> listAdjacence;
	
	/**
	 * créer la liste des arretes dont le currentSommet est le départ et l'arrivée est un sommet adjacent a currentSommet
	 * @param boatPosition
	 * @param currentSommet
	 * @param recifs
	...
	/**
	 * retourne la liste d'arrete si currentSommet se trouve sur une des extremite visible par le bateau
	 * 
	 * @param boatPosition
	 * @param currentSommet
	 * @param checkpoint
	...
	private List<Arrete> listArretePointOnedge(IPoint boatPosition,IPoint currentSommet,IPoint checkpoint ,List<Recif> recifs) {
		...
		List<Arrete> list =new ArrayList<Arrete>();
		if(currentSommet.x()==boatPosition.x()-(VISIBLITEMAX/2) && currentSommet.y()==boatPosition.y()-(VISIBLITEMAX/2)) {
			list.addAll(List.of(
					new Arrete(currentSommet,HC),
					new Arrete(currentSommet,HD),
					new Arrete(currentSommet,MD)));

		}else if(currentSommet.x()==boatPosition.x()-(VISIBLITEMAX/2) && currentSommet.y()==boatPosition.y()+(VISIBLITEMAX/2)) {
			list.addAll(List.of(
					new Arrete(currentSommet,BC),
					new Arrete(currentSommet,BD),
					new Arrete(currentSommet,MD)));
		}else if(currentSommet.x()==boatPosition.x()+(VISIBLITEMAX/2) && currentSommet.y()==boatPosition.y()-(VISIBLITEMAX/2)) {
			list.addAll(List.of(
					new Arrete(currentSommet,BG),
					new Arrete(currentSommet,HC),
