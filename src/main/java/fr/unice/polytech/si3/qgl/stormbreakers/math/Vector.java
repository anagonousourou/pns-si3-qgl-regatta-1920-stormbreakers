...
public class Vector {
    ...
    /**
     * Mutiplie les coords du vecteur par le facteur passé en parametre ne modifie
     * pas le vecteur
     * 
    ...
    /**
     * Returns vector orientation in radians Bounds : [0,2Pi]
     * 
    ...
    public Vector extendBy(double margin) {
        if (Utils.almostEquals(0.0, this.norm()))
            throw new UnsupportedOperationException("Cannot extend a vector of norm 0");
        double currentMag = norm();
        return this.setScaleTo(currentMag + margin);
    }

    /**
     * Changes the vectors scale if the new scale is negative, the vector direction
     * will be flipped
     * 
    ...
    public Vector setScaleTo(double newScale) {
        if (Utils.almostEquals(0.0, this.norm()))
            throw new UnsupportedOperationException("Cannot set scale for a vector of norm 0");
