...
public class LineSegment2DTest {
	...
	void pointFromSegmentParameterTest() {
		...
		Point2D B = new Point2D(6,66);
		LineSegment2D nonVerticalLineSegment = new LineSegment2D(A,B);
		assertTrue(Utils.almostEquals(A, nonVerticalLineSegment.pointFromSegmentParameter(0), delta));
		Point2D tmp1 = nonVerticalLineSegment.getMiddle();
		Point2D tmp2 = nonVerticalLineSegment.pointFromSegmentParameter(0.5);
		assertTrue(Utils.almostEquals(tmp1, tmp2, delta));
		assertTrue(Utils.almostEquals(B,nonVerticalLineSegment.pointFromSegmentParameter(1), delta));

		Point2D C = new Point2D(6,42);
		Point2D D = new Point2D(6,0);
		LineSegment2D verticalLineSegment = new LineSegment2D(C,D);
		assertTrue(Utils.almostEquals(C,verticalLineSegment.pointFromSegmentParameter(0), delta));
		assertTrue(Utils.almostEquals(verticalLineSegment.getMiddle(),verticalLineSegment.pointFromSegmentParameter(0.5), delta));
		assertTrue(Utils.almostEquals(D,verticalLineSegment.pointFromSegmentParameter(1), delta));
	}
	...
	void testSegmentParameterReciprocity() {
		...
		for (double n=0; n<=1; n+=step){
			...
			assertTrue(Utils.almostEquals(point2D, lineSegment2D.pointFromSegmentParameter(lineSegment2D.segmentParameterOf(point2D)), delta));
		}
	}
}